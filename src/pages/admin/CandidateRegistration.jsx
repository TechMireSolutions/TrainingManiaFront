...
const CandidateRegistration = () => {
  ...
  const handleDeleteCandidate = (emailToDelete) => {
    if (window.confirm(`Are you sure you want to delete ${emailToDelete}? This will also remove their enrollments.`)) {
      // 1. Remove from candidates list
      ...
      // 2. Remove from enrollments
      try {
        const enrollments = JSON.parse(localStorage.getItem('enrollments') || '[]');
        const updatedEnrollments = enrollments.filter(e => e.candidate !== emailToDelete);
        localStorage.setItem('enrollments', JSON.stringify(updatedEnrollments));
      } catch (error) {
        console.error("Failed to update enrollments during deletion:", error);
      }
    }
  };
